...
class DBInitializer {
  ...
  static async init() {
    if (!fs.existsSync(settings.dbPath)) {
      ...
      await this.#runQuery(`CREATE TABLE IF NOT EXISTS ${Tables.CALCULATION} (
        Id INTEGER,
    ...
    if (fs.existsSync(settings.dbPath)) {
      ...
      const scriptsRoot = path.resolve('./src/Data.Deploy/Scripts');
      if (fs.existsSync(scriptsRoot)) {
        ...
        for (const sprintFolder of scriptFolders) {
          ...
          for (const sqlFile of sqlFiles) {
            ...
            const found = await this.#getRecord(query, [sprintFolder, sqlFile]);
            if (!found) {
